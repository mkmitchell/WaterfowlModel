...
combined$OWNER <- sub("^$", "Private", combined$OWNER)

# If ST_FED is private and Z_HARVESTE == 1 make it 100
combined$Z_HARVESTE = ifelse((combined$OWNER == "Private") & (combined$Z_HARVESTE == 1), 100, combined$Z_HARVESTE)

# IF ST_FED is private and cover is woody wetlands set z_red_oak to 20
combined$Z_RED_OAK_ = ifelse((combined$OWNER == "Private") & (combined$COVER_TYPE == "woody wetlands"), 20, combined$Z_RED_OAK_)
